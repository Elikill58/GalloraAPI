...
public class VNegativityCommand implements Command {
	...
	public void execute(CommandSource source, @NonNull String[] args) {
		if (source instanceof Player && !Perm.hasPerm(((Player) source).getUniqueId(), Perm.ADMIN)) {
	...
	private static boolean hasAdminPermission(CommandSource source) {
		return !(source instanceof Player) || Perm.hasPerm(((Player) source).getUniqueId(), Perm.ADMIN);
	}
}
